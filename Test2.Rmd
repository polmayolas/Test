...
We noticed that there is 1 outliers in our independent variable, the one that we want to predict (Distance). So, before we create our model we need to remove this outlier from the data set.We just did using the "which" and "out" functions. 
...
boxplot(cars2$Distance)

```

We then created another data set "cars2" for if we still need the original one for something. We prefer to have 2 different data sets, instead of doing changes on the original data set.
...
Time to use our model with the data that we reserved for testing. 
...
```{r}
knitr::kable(PredictionCars)
```

```{r}
TestSetFinal <- cbind(testSet$Brand, testSet$Distance, PredictionCars)
knitr::kable(TestSetFinal)
```
